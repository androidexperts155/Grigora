...
class OrderController extends Controller
{
    ...
    public function addInGroupCart(Request $request){
    	...
    	try{
            ...
            if($check){
	            ...
	            if($request->has('cart_type') && !empty($request->cart_type)){
    ...
    public function addCart(Request $request){
    	...
    	try{
            ...
            $userImage = User::where('role', '<>', '1')->pluck('image', 'id')->toArray();
            $settings = Setting::where('id', '1')->first();
            $check = Cart::where('user_id', $user->id)->where('group_order', '<>', '1')->where('status','1')->first();
            if($check){
    ...
    public function updateCartItemQty(Request $request){
    	try{
	        ...
	        $cart = Cart::where('id', $cartId)->first();
	        if($cart){
	        	$checkCartUser = CartUser::where('cart_id', $cartId)->where('user_id', $user->id);
        		if($checkCartUser){
    ...
    public function cartItemList($id){
    	try{
    		$user = Auth::user();
    		$cart= Cart::where("user_id",$user->id)->where('group_order', '<>', '1')->where('status', '1')->first();
    		//return $cart;
    		$cartItems = CartItemsDetail::where('cart_id', $cart['id'])->where('item_id', $id)->get()->toArray();
    	...
    	try{
            ...
            if($cart){
                ...
                if($result){
                    ...
                    $cart['fullTime'] = $restaurantInfo->fullTime;
    ...
    public function viewCart(Request $request){
    	try{
            ...
            $userName = User::where('role', '<>', '1')->pluck('name', 'id')->toArray();
            $userImage = User::where('role', '<>', '1')->pluck('image', 'id')->toArray();
    		$cart = Cart::where("user_id",$user->id)->where('group_order', '<>', '1')->where('status', '1')->with("cart_details")->first();
    		//return $cart;
    		$restaurantInfo = User::where('id', $cart['restaurant_id'])->first();
            		...
            		$wallet = $user['wallet']+$order['final_price'];
	   				...
	   				if($user->notification == '1'){
		                ...
		                if($userTokens){
		                    foreach ($userTokens as $tokenKey => $userToken) {
		                        if($userToken['device_type'] == '0'){
		                            $sendNotification = $this->sendPushNotification($userToken['device_token'],$msg,$deta = array("notification_type" => '8'));    
		                        }
		                        if($userToken['device_type'] == '1'){
		                            $sendNotification = $this->iosPushNotification($userToken['device_token'],$msg,$deta = array("notification_type" => '8'));    
		                        }
		                    }
		                }
