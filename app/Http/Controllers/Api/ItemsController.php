            ...
            if($item->save()){
                ...
                foreach ($itemCategories as $key => $itemCategorie) {
                    ...
                    foreach ($itemCategorie->item_sub_category as $key1 => $itemCategori) {
                        ...
                        $itemSubCat->save();
                        // if(is_array($itemCategori->item_sub_sub_category) && !empty($itemCategori->item_sub_sub_category)){
                        //     foreach($itemCategori->item_sub_sub_category as $key2 => $itemSubCat){
                        //         $englishWords = array($itemSubCat->name);
                        //         $frenchWords = $this->translation($englishWords);
                        //         $itemSubSubCat = new ItemSubSubCategory;
                        //         $itemSubSubCat->item_sub_cat_id = $itemSubCat->id;
                        //         $itemSubSubCat->name = $itemSubCat->name;
                        //         $itemSubSubCat->french_name = $frenchWords[0];
                        //         $itemSubSubCat->add_on_price = $itemSubCat->add_on_price;
                        //         $itemSubCat->save();
                        //     }
                        // }
                    }
